...
const CHAPTER = 2;
const CHAPTER_START_NODE = 'cns';
const CHAPTER_NODE_IDS   = new Set([
  'cns', 'brain_parts', 'cerebrum'
]);
const CHAPTER_TITLE = 'Chapter 2: The Journey';
...
// ─── Chapter 2 requires Chapter 1 to be completed first ───────────────────────
...
const VisualNovelChapter2 = () => {
  ...
  // ── Check prerequisite & load progress ─────────────────────────────────────
  useEffect(() => {
    if (!userId) return;

    // Require Chapter 1 to be completed
    const ch1Done = localStorage.getItem(`vn_chapter1_completed_${userId}`);
    if (ch1Done !== 'true') {
      setChapter1Locked(true);
      setLoading(false);
      return;
    }
  ...
  // ── Render ──────────────────────────────────────────────────────────────────
  if (loading && !currentNode && !chapter1Locked) return <div>Loading…</div>;
  ...
  return (
    <GameContainer backgroundPath={backgroundPath}>
      {/* Locked state — Chapter 1 not completed */}
      {chapter1Locked && (
        <LockedOverlay>
          <h2>Chapter Locked</h2>
          <p>Complete Chapter 1 before playing Chapter 2.</p>
          <NavButton onClick={() => navigate('/lessons')}>Return to Lessons</NavButton>
        </LockedOverlay>
      )}

      {/* Chapter intro banner */}
      {!chapter1Locked && showChapterBanner && (
        <ChapterBanner>
          <h1>{CHAPTER_TITLE}</h1>
...
export default VisualNovelChapter2;
